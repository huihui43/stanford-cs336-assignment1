...
# 定义主进程中的全局变量
list_global_word_freqs = []
list_global_word_splits = []
list_global_pair_freqs = []
list_global_word_bytes = []
...
#def update_result(word_freq, word_splits, pair_freqs, word_bytes):
def update_result(res):
    global list_global_word_freqs
    global list_global_word_bytes
    global list_global_word_splits
    global list_global_pair_freqs

    list_global_word_freqs.append(res[0])
    list_global_word_splits.append(res[1])
    list_global_pair_freqs.append(res[2])
    list_global_word_bytes.append(res[3])
        ...
        while cur and nxt: # while i < n_split - 1
            if cur.data == a and nxt.data == b: # 找到匹配
                ...
                if prev:
                    ...
                    pair_freq[c, index] =  pair_freq.get((c,index), 0) + freq # add new merge
                if nxt.next: 
                    c = nxt.next.data
                    pair_freq[b, c] -= freq
...
def trainer(
    ...
    if 1:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            pool.apply_async(pretokenize_chunk,args=(input_path,start,end,pre_tokenizer),callback=update_result) 
    ...
    global_word_freqs, global_word_splits, global_pair_freqs, global_word_bytes = summarize(list_global_word_freqs, list_global_word_splits, list_global_pair_freqs, list_global_word_bytes)
